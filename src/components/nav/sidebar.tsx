...
export function Sidebar({ homeName, members = [] }: { homeName?: string; members?: SidebarMember[] }) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 hidden w-64 shrink-0 -translate-x-full flex-col gap-5 border-r border-border bg-card px-4 py-5 shadow-sm transition-transform duration-200 ease-out md:flex",
          open && "translate-x-0"
        )}
      >
